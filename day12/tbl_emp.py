...
def select_emp():
    ...
    sql = "SELECT * FROM employee ORDER BY emp_id"  #db 검색, 오름차순(osc 생략됨)
...
def insert_emp():   #자료 추가
    ...
    # 입력방법 1 칼럼값을 직접 입력
    # sql = "INSERT INTO employee VALUES ('e1001', '추신수', 42, 40000)" #data 추가
    # 방법 2 - ?기호 사용 - 동적 입력
    # sql = "INSERT INTO employee VALUES ('?', '?', ?, ?)"
    # cur.execute(sql, ('e1004', '박인비', 33, 30000))
    conn.commit()   #commit 실행 - 필수(데이터에 변경이 생겼을때 반드시 명시)
    conn.close()
...
def select_one():   #특정 자료 검색
    ...
    cur.execute(sql, ('e1002', ))     #튜플 구조는 1개를 명시할 때 콤마 찍어야 함
